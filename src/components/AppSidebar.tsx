...
import { Plus, MessageCircle, Sun, Moon, Trash2, LogOut, User, Menu } from 'lucide-react'
...
export function AppSidebar({ 
  ...
  return (
    <TooltipProvider>
      <Sidebar className="border-r border-aloo-border bg-aloo-sidebar-bg">
        <SidebarHeader className="p-4 border-b border-aloo-border">
          {/* AlooChat Branding */}
          <div className="flex items-center gap-3 mb-4">
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className="text-lg font-semibold text-aloo-text-primary">
                AlooChat
              </span>
            </Link>
          ...
          <Button 
            onClick={onNewChat}
            className="w-full bg-aloo-accent hover:bg-aloo-accent-hover text-white"
          >
                ...
                <SidebarMenu>
                  {chats.map((chat) => (
                    <SidebarMenuItem key={chat.id}>
                      <div className={`group flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all hover:bg-aloo-accent hover:text-white ${
                        chat.id === activeChatId 
                          ? 'bg-aloo-accent text-white' 
                          : 'hover:bg-aloo-accent/10'
                      }`}>
                        <div
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <Badge
                              variant="secondary"
                              className={`text-xs px-2 py-0 h-5 border-0 ${
                                chat.id === activeChatId
                                  ? 'bg-white/20 text-white'
                                  : 'bg-aloo-accent/20 text-aloo-accent'
                              }`}
                            >
        ...
        <SidebarFooter className="p-4 border-t border-aloo-border">
          {user && (
            <DropdownMenu>
